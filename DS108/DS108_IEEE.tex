...
\subsection{Đóng Gói Tensor Chuỗi Thời Gian}

\subsubsection{Phân Chia 70/10/20 Với Embargo Gap}
...
\subsubsection{Kiến Trúc Tensor Động / Tĩnh}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Thiết Kế Nhãn Mục Tiêu}
...
Bảng~\ref{tab:dataset_stats} tóm tắt thống kê bộ dữ liệu DS108-AgriCommodity.
\begin{table}[H]
...
Market: yfinance; Weather: Open-Meteo (CC BY 4.0)~\cite{zippenfenig2023};
CPI: BLS~\cite{bls2026}; Farming: Synthetic.
...
Mô hình không phải lời khuyên đầu tư. Cần quản trị rủi ro và paper trading
$\geq$12 tháng trước khi deploy production. Không có PII trong dataset.
