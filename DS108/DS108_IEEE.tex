...
%% ── Font: TeX Gyre Termes (Times clone với full Vietnamese glyph) ─────────────
\usepackage{fontspec}
...
v7 threshold 2.5\%  & \textbf{0.709} & \textbf{0.662} & --- \\
v1 threshold 5\%    & 0.564          & 0.443          & --- \\
\midrule
...
Tuy nhiên, center=True inflate vẫn thấp hơn Stacking clean (0.709/0.662)
...
\caption{Hurdle model: scatter predicted vs.\ actual return. Hàng trên:
Corn Daily; hàng dưới: Coffee Daily. Cột trái~(a): Stage~2b --- down trades,
predicted $|\text{return}|$ vs.\ actual $|\text{return}|$; Corn Daily
$r{=}{+}0.371^{***}$ ($p{<}0.0001$, $n{=}164$). Cột phải~(b): Full hurdle
combined (signed). Stage~2a bị loại khỏi hình vì model dừng tại
\texttt{best\_iteration}{=}1 --- chỉ dự đoán hằng số gần trung bình, không
có thông tin phân biệt biên độ tăng. Đường thẳng: OLS. Sao:
$^{*}{<}0.05$, $^{**}{<}0.01$, $^{***}{<}0.001$.}
\label{fig:hurdle}
\end{figure}
