...
%% Better justification for Vietnamese two-column layout
\tolerance=2000
...
\and
\IEEEauthorblockN{Kiên Huỳnh Chí}
\IEEEauthorblockA{
\textit{Khoa Khoa học và Kỹ thuật Thông tin}\\
\textit{Trường Đại học Công nghệ Thông tin, ĐHQG-HCM}\\
Thành phố Hồ Chí Minh, Việt Nam\\
24520880@gm.uit.edu.vn}
}
\maketitle
% ─────────────────────────────────────────────────────────────────────────────
\begin{abstract}
...
bốn đóng góp chính:
%% [FIX A] Danh sách đóng góp được làm giàu với "tại sao nó khó"
\begin{enumerate}
\item \textbf{Tích hợp 4 luồng dữ liệu bất đồng nhất} --- thị trường, vi
  khí hậu, kinh tế vĩ mô và lịch nông nghiệp --- mỗi luồng có tần suất,
  đơn vị và độ trễ công bố khác nhau, đòi hỏi chiến lược đồng bộ riêng biệt;
\item \textbf{Phát hiện và sửa lỗi rò rỉ dữ liệu mức P0} trong pipeline hồi
  quy: cột \texttt{target\_mc\_*} bị bỏ sót khỏi danh sách loại trừ, tạo ra
  kết quả ảo $r$=0.986 trước khi được khắc phục về $r$=0.076;
\item \textbf{Xác thực thống kê độ trễ sinh học} (34 tuần cà phê, 9 tuần ngô)
  qua CCF bootstrap, thay vì hardcode theo kinh nghiệm;
\item \textbf{Kiến trúc LLM-as-feature-extractor} dùng Claude API trích xuất
  đặc trưng có cấu trúc từ báo cáo USDA/PSD, cải thiện AUC $+$8.7~pp trên
  Coffee Weekly.
\end{enumerate}
...
\end{itemize}
Open-Meteo~\cite{zippenfenig2023} cung cấp 5h biến khí tượng theo ngày:
...
Hiệu quả của quá trình này được minh họa tại Hình~\ref{fig:miqr_weather}. Thuật
toán đã tự động cắt bỏ các đỉnh nhiễu phi lý (do lỗi thiết bị cảm biến vi khí hậu)
và thay thế bằng dữ liệu quá khứ gần nhất (\texttt{ffill}), giúp bảo toàn hoàn
toàn tính liên tục vật lý của biến nhiệt độ.
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig2_miqr.pdf}
\caption{Tác động của bộ lọc MIQR đối với dữ liệu nhiệt độ cực đại tại
vùng Cerrado Baiano (năm 2010). Các điểm đánh dấu \textbf{X} thể hiện những quan
sát bị lỗi (outliers) đã được thuật toán nhận diện và hiệu chỉnh bằng \texttt{ffill},
đảm bảo nguyên tắc không sử dụng thông tin tương lai.}
\label{fig:miqr_weather}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig3_ACU.pdf}
\caption{So sánh giá đóng cửa của hợp đồng Cà phê (KC=F) trước và sau khi đi
qua bộ lọc ACU (giai đoạn 2014--2016). Các điểm Flash Crash
được hệ thống nhận diện tự động và nội suy thành công, minh chứng cho việc
áp dụng thuật toán ACU bảo toàn tính toàn vẹn của dữ liệu tài chính.}
\label{fig:acu_before_after}
\end{figure}
...
%% [FIX B] Giải thích Corn Weekly LLM thua Synthetic — frequency mismatch
Kết quả trên Corn Weekly là ngoại lệ đáng chú ý: LLM (0.548) thua Synthetic
(0.598). Nguyên nhân là đặc trưng LLM trích xuất từ dữ liệu USDA có tần suất
tuần, khi tổng hợp lên độ phân giải tuần sẽ mất đi tính biến thiên theo
từng ngày. Ngược lại, cờ nhị phân tổng hợp (\texttt{is\_harvest},
\texttt{is\_planting}) encode chính xác chu kỳ tháng --- đủ phân giải cho
mô hình tuần. Phát hiện này gợi ý rằng hiệu quả của LLM phụ thuộc vào sự
khớp giữa tần suất nguồn dữ liệu và horizon dự báo.
\begin{table}[t]
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig4_ccf.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig5_split.pdf}
...
%% [FIX C — phần 1] LightGBM: mở rộng lý do scale_pos_weight cap = 5.0
...
đoạn 2010--2025. Tham số \texttt{scale\_pos\_weight} được giới hạn trần ở mức
5.0 để tránh mất ổn định gradient khi tỷ lệ âm/dương vượt quá 10:1 ---
ngưỡng mà theo thực nghiệm dẫn đến diverge sau vài epoch đầu.
...
ensemble.
%% [FIX C — phần 2] LSTM: mở rộng lý do chọn window sizes {14,30,45}
\textbf{LSTM Hybrid.} Kiến trúc của mô hình:
\begin{equation}
\text{LSTM}(128{\to}64) \oplus X_\text{stat}
...
$W \in \{14,30,45\}$ ngày --- tương ứng 2 tuần, 1 tháng và 6 tuần lịch sử,
bao phủ các chu kỳ kỹ thuật ngắn, trung và dài hạn phổ biến trong phân tích
hàng hóa.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig6_equity.pdf}
...
\item[b] Lần lượt $\Delta$ Coffee ($+$0.248) và Corn ($+$0.228). center=True
...
\end{threeparttable}
\end{table}
\subsection{Bootstrap Sharpe CI Và Phân Tích Chi Phí Giao Dịch}
Để đánh giá tính ổn định thống kê, chúng tôi áp dụng bootstrap Sharpe CI
($n$=10,000 lần lấy mẫu có hoàn lại) cho ba chiến lược production.
Bảng~\ref{tab:bootstrap} trình bày kết quả, trong đó production candidate
được chọn là \textbf{Coffee Weekly Binary RF} (Sharpe=0.962, alpha $+$16.4~pp)
--- nhất quán với kết quả backtesting và equity figure.
\begin{table}[t]
\caption{Bootstrap Sharpe CI ($n$=10,000) --- Production Strategies}
\label{tab:bootstrap}
\centering
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Dataset} & \textbf{Model} & \textbf{95\% CI} & \textbf{P(S$>$0)} & \textbf{P(S$>$1)} \\
\midrule
Coffee Daily  & Stack     & [0.62,\;3.65] & ---   & 93.2\% \\
Coffee Weekly & Binary RF & [-0.24,\;2.18] & 94.6\% & 48.6\% \\
Corn Daily    & MC RF L/S & [-0.60,\;2.31] & 86.4\% & --- \\
\bottomrule
\end{tabular}
\end{table}
CI của Coffee Weekly Binary RF là $[-0.24,\;2.18]$ với P(Sharpe$>$0)=94.6\%
--- xác nhận chiến lược có lợi nhuận dương với xác suất cao, dù giới hạn dưới
vẫn âm do biến động lớn trong năm 2025. Corn Daily ($[-0.60,\;2.31]$,
P$>$0=86.4\%) đủ khuyến khích để tiếp tục theo dõi, nhưng chưa đủ để triển
khai production ngay.
Bảng~\ref{tab:cost} phân tích độ nhạy chi phí giao dịch đối với Coffee Daily
Stack (cấu hình có Sharpe tuyệt đối cao nhất):
\begin{table}[t]
\caption{Phân Tích Độ Nhạy Chi Phí Giao Dịch --- Coffee Daily Stack}
\label{tab:cost}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Chi phí/lệnh} & \textbf{Sharpe} & \textbf{Verdict} \\
\midrule
0.0\% (baseline) & 2.154 & Viable \\
0.1\%            & 1.821 & Viable \\
0.2\%            & 1.488 & Viable \\
0.3\%            & 1.155 & Degraded \\
\bottomrule
\end{tabular}
\end{table}
Sharpe vẫn dương ngay cả ở mức chi phí 0.3\%, cho thấy chiến lược Coffee Daily
Stack có đủ biên lợi nhuận để chịu được các khoản chi phí giao dịch thực tế.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig8_hurdle.pdf}
