...
def fetch_vix(start_date, end_date):
    print("Đang tải VIX (CBOE Volatility Index ^VIX)...")
    vix_raw = yf.download("^VIX", start=start_date, end=end_date, auto_adjust=True)
    if not vix_raw.empty:
        # Extract Close column — handles both flat and MultiIndex column structures
        if isinstance(vix_raw.columns, pd.MultiIndex):
            close_col = next(c for c in vix_raw.columns if c[0] == "Close")
            close_vals = vix_raw[close_col].values
        else:
            close_vals = vix_raw["Close"].values
        out = pd.DataFrame({"Date": vix_raw.index, "VIX_Close": close_vals})
        os.makedirs("data/raw/macro", exist_ok=True)
        out.to_csv("data/raw/macro/vix_index.csv", index=False)
        print(f"-> Đã lưu VIX: {len(out)} rows → data/raw/macro/vix_index.csv")
    else:
        print("Lỗi: Không tải được VIX (^VIX)")
...
if __name__ == "__main__":
    ...
    # Lấy VIX — market fear gauge (macro context cho corn)
    fetch_vix(START_DATE, END_DATE)