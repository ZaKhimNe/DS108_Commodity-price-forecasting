...
def lgbm_null_importance_selection(df, name, target_col='target', iv_threshold=0.0):
    ...
    # Dùng LGBMRegressor vì TARGET_MODE == "regression"
...
def process_module_5(file_name, window_sizes=[14, 21, 30], static_cols_input=[]):
    ...
    train_df = lgbm_null_importance_selection(train_df, name=file_name, target_col=target_col, iv_threshold=0.01)
    ...
    print(f"   - Target mean  Train={train_df[target_col].mean():.4f}  "
...
if __name__ == "__main__":
    ...
    print("\n[V] 13c TENSOR PACKING REGRESSION HOÀN TẤT!")
    print(f"    Output dir: {OUTPUT_BASE_DIR}")
    print("    y shape: (N,) scalar — dùng cho HuberLoss regression models (16c, 17c).")
